...
- **Specs dir resolution**: cwd-relative by default (`./specs`), overridable via `--specs` flag or `VIDEO_DVR_SPECS` env var. `__dirname`-relative resolution replaced entirely.
- **CLI entry**: new `bin/video-dvr.js` shebang script; `app.js` stays the main module.
...
1. Is `video-dvr` available on npm, or do we need a scoped package name?
...
- `bin/video-dvr.js` — new CLI shebang entry point
