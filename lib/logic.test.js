...
describe('resolveRenditionErrors', () => {
  it('returns empty object when no rendition errors', () => {
    const ops = [{ op: 'startup', delay: 5 }, { op: 'playback', time: 30 }];
    assert.deepEqual(resolveRenditionErrors(ops), {});
  });

  it('returns map of rendition name to error code', () => {
    const ops = [
      { op: 'playback', time: 30 },
      { op: 'error', code: 404, rendition: 'low' },
    ];
    assert.deepEqual(resolveRenditionErrors(ops), { low: 404 });
  });

  it('defaults to 500 when no code specified', () => {
    const ops = [{ op: 'error', rendition: 'low' }];
    assert.deepEqual(resolveRenditionErrors(ops), { low: 500 });
  });

  it('handles multiple rendition errors', () => {
    const ops = [
      { op: 'error', code: 404, rendition: 'low' },
      { op: 'error', code: 503, rendition: 'mid' },
    ];
    assert.deepEqual(resolveRenditionErrors(ops), { low: 404, mid: 503 });
  });

  it('returns empty object for null operations', () => {
