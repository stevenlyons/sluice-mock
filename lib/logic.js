...
function resolveRenditionErrors(operations) {
  ...
  for (const op of operations) {
    if (op.op === 'playback') {
      currentSegment += opSegmentCount(op);
    }
...
function parseSpecification(specPath) {
  ...
  return opString.split('-').map(parseOp).filter(Boolean);
}

function parseOp(opString) {
  const data = opString.length > 1 ? parseInt(opString.substring(1)) : undefined;
  switch (opString.charAt(0)) {
    case 's': return { op: 'startup', delay: data || 5 };
    case 'p': return { op: 'playback', time: data || 30 };
    case 'r': return { op: 'rebuffer', delay: data || 30 };
    case 'e': return { op: 'error', ...(data && { code: data }) };
    default:  return null;
  }
}
    ...
    switch (op.op) {
      ...
      case 'error': {
        ...
        if (el && el.segment === currentSegment) {
          el.error = op.code || '500';
        } else {
          timeline.push({ segment: currentSegment, error: op.code || '500' });
        }
        currentSegment += opSegmentCount(op);
        break;
      }
...
// Returns the number of segments an operation occupies in the timeline.
function opSegmentCount(op) {
  if (op.op === 'playback') return Math.ceil(op.time / segmentLength);
  if (op.op === 'error') return 1;
  return 0;
}

function calculateMediaLength(operations) {
  if (!operations) return;
  return operations.reduce((total, op) => total + opSegmentCount(op) * segmentLength, 0);
}
