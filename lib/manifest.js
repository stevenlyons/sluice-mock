...
function buildMasterPlaylist(renditions) {
  ...
  renditions.forEach((r, i) => {
    ...
    const renditionFile = r.name
      ? `rendition-${r.name}.m3u8`
      : `rendition-${i}.m3u8`;
...
#EXT-X-MAP:URI="init.mp4"`;
  ...
  let segments = '';
  const segCount = Number(new Big(mediaLength).div(SEG).round(0, 3));
  for (let i = 0; i < segCount; i++) {
    const segFile = renditionName
      ? `seg-${renditionName}-${i + 1}.m4s`
      : `seg-${i + 1}.m4s`;
...
function buildDashMPD(mediaLength, renditions) {
  const representations = renditions
    .map((r, i) => {
      ...
      const id = r.name || i + 1;
      const mediaTemplate = r.name
        ? `seg-${r.name}-$Number$.m4s`
        : `seg-$Number$.m4s`;
      return `      <Representation id="${id}" bandwidth="${r.bandwidth}" codecs="avc1.640020" width="${width}" height="${height}">
        <SegmentTemplate initialization="init.mp4" media="${mediaTemplate}" duration="${SEG.mul(1000)}" timescale="1000" startNumber="1"/>
      </Representation>`;
    })
    .join('\n');
