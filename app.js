  ...
  switch (checkRequestType(filename)) {
    case 'media':
      ...
      outputString(
        ...
        buildMasterPlaylist(spec.renditions)
      );
      break;
    case 'rendition': {
      console.log('Rendition request');
      const renditionName = extractRenditionName(filename);
      const renditionError =
        renditionName && spec.renditionErrors.playlist[renditionName];
      if (renditionError) {
        ...
        outputString(
          ctx,
          'application/x-mpegURL',
          buildRenditionPlaylist(spec.mediaLength, renditionName)
        );
      }
      break;
    }
    case 'dash-manifest': {
      ...
      outputString(
        ...
        buildDashMPD(spec.mediaLength, spec.renditions)
      );
      break;
    }
    case 'segment': {
      console.log('Segment request');
      const { rendition: segRendition, segment: segNum } =
        extractRenditionFromSegment(filename);
...
async function loadSpecification(filepath) {
  ...
  try {
    ...
    return {
      timeline,
      mediaLength,
      renditions: resolveRenditions(json),
      renditionErrors,
    };
  } catch {
    ...
    return {
      timeline,
      mediaLength,
      renditions: resolveRenditions({ timeline }),
      renditionErrors: { playlist: {}, segment: {} },
    };
  }
}
...
async function processSegment(
  ...
  const { segment: requestedSegNum } = extractRenditionFromSegment(
...
async function outputFile(
  ...
  if (fstat.isFile()) {
    ...
    const ext =
      temp.charAt(0) === '.' ? extractMimetype(temp.substring(1)) : temp;
